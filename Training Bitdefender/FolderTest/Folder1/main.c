...
int parsePeFile(MAPPED_FILE *mf) {
	...
	if (pNt->OptionalHeader.DataDirectory[0].VirtualAddress == NULL) {
	...
	if (pNt->OptionalHeader.DataDirectory[1].VirtualAddress == NULL) {
...
int traverseDirectory(char* path) {
...
int main()
{
	char a[] = "D:\\Facultate\\proiecte\\Training Bitdefender\\*";
	traverseDirectory(a);
	return 0;
}